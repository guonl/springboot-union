...
/**
 * Created by guonl
 * Date 2018/6/27 下午12:09
 * Description: 恶意用户检测拦截器
 */
@Component
public class MyUserInterceptor extends HandlerInterceptorAdapter {
    @Autowired
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        // 超过限定时间内的访问频率
        if (requestRecords!= null && requestRecords.size() + 1 >= systemConfig.getIpBlackTimes() && (System.currentTimeMillis()
                - ((UserRequestRecord)requestRecords.get(requestRecords.size() - 1)).timestamp < systemConfig.getUserBlackSeconds() * 1000)) {
     ...
     * 用户访问记录
     */
    public static class UserRequestRecord implements Serializable{
        /**
         * 手机号，唯一标志用户身份
         */
