...
var goods = {
    ...
    URL: {
        ...
        exposer: function (goodsId) {
            return '/sec/' + goodsId + '/getSecKillGoodsLink';
        },
        execution: function () {
            return '/sec/' +'miaosha';
        },
        queryResult: function () { //查询秒杀结果
            return '/sec/miaoshaResult';
        },
        order: function () { //下单
            return '/sec/order';
        },
		...
		$.ajax({
			...
			success: function(data, status) {
				if(status == "success" && data.code == 0 && data.data) {
					...
					// 下单token存起来，用于下单
					$.cookie('token', data.data, {expires: 1, path: '/sec'});
    ...
    detail: {
        ...
        init: function (params) {
            ...
            if (!goods.validatePhone(killPhone)) {
                ...
                $('#killPhoneBtn').click(function () {
                    ...
                    if (goods.validatePhone(inputPhone)) {
                        //电话写入cookie(7天过期)
                        $.cookie('killPhone', inputPhone, {expires: 7, path: '/sec'});
                        //验证通过　　刷新页面
            ...
            $.get(goods.URL.now(), {}, function (result) {
